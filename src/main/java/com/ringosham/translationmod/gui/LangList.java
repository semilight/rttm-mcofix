...
public class LangList extends GuiScrollingList {
    ...
    //mark blocked languages so you can see what you picked while toggling
    ...
    protected void drawSlot(int listIndex, int right, int top, int height, Tessellator var5) {
        ...
        boolean skipped = markSkipped && ConfigManager.INSTANCE.isLanguageSkipped(lang);
        if (skipped) {
            langName = "* " + langName;
        }
        //Overflow
        ...
        if (top > this.top + this.listHeight)
            return;
        parent.mc.fontRenderer.drawString(langName, left + 5, top, skipped ? 0x55FF55 : 0xffffff);
    }

    public void setMarkSkipped(boolean markSkipped) {
        this.markSkipped = markSkipped;
    }
