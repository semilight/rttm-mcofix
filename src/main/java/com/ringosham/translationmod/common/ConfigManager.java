...
public class ConfigManager {
    ...
    //old regexes from previous versions, only here so existing configs get updated
    private static final String[] OLD_VANILLA_REGEXES = {
            "<(\\w+)> ",
            "<[^\\w>]*(\\w+)> ",
            //this one matched mid line on (STAFF) messages and mangled the header
            "(?:\\[(?:DSC|IRC|TG)\\] )*<[^\\w>]*(\\w+)> (?:\\[(?:DSC|IRC|TG)\\] )*"
    };
    //the current chat regex. \\w can't match the asterisks in names like <*****Player>, so [^\\w>]* skips them.
    //also eats [DSC]/[IRC]/[TG] relay tags and a leading (STAFF) so they don't end up in the translation.
    private static final String NEW_VANILLA_REGEX = "(?:\\(STAFF\\) )?(?:\\[(?:DSC|IRC|TG)\\] )*<[^\\w>]*(\\w+)> (?:\\[(?:DSC|IRC|TG)\\] )*";
    ...
    //MCO [TR] tags, so they aren't translated twice (thanks slime!!)
    private static final String[] DEFAULT_SERVER_TAGS = {"[TR]", "[TR->"};
    public static final String FALLBACK_ENGINE = "selfhosted";
    ...
    //kept so we can check a save actually happened
    ...
    private String nllbUrl;
    private String selfHostedFallbackLang;
    private int selfHostedMinConfidence;
    private int selfHostedTimeout;
    private int selfHostedMinChars;
    private int selfHostedMaxSimilarity;
    private String selfHostedApiKey;
    private List<String> skipLanguages;
    private List<String> serverTranslationTags;
    //set when google dies
    ...
    public void syncConfig() {
        ...
        //Arrays.asList is fixed size and the loop below needs to change these
        ...
        //update old configs so decorated names and relay tags start working
        boolean regexMigrated = false;
        for (int i = 0; i < regexList.size(); i++) {
            String trimmed = regexList.get(i).trim();
            for (String old : OLD_VANILLA_REGEXES) {
                if (trimmed.equals(old.trim())) {
                    regexList.set(i, NEW_VANILLA_REGEX);
                    regexMigrated = true;
                    break;
                }
            }
        ...
        translationEngine = config.getString("translationEngine", Configuration.CATEGORY_GENERAL, DEFAULT_ENGINE, "Translation engine used");
        //languages in your "never translate" list. the target-language skip only ever helped native english speakers.
        skipLanguages = new ArrayList<>(Arrays.asList(config.getStringList("skipLanguages", Configuration.CATEGORY_GENERAL, new String[0], "Languages never to translate automatically, as ISO codes or English names (e.g. es, Spanish). The target language is always skipped and does not need listing. Explicit actions; signs, /tr and retranslate, ignore this list")));
        //already translated chat
        serverTranslationTags = new ArrayList<>(Arrays.asList(config.getStringList("serverTranslationTags", Configuration.CATEGORY_GENERAL, DEFAULT_SERVER_TAGS, "Messages starting with any of these are already translated by the server and are left alone. Matching ignores case and leading spaces. Empty this list to translate them anyway. Only applies to automatic chat: signs, /tr and retranslate still work")));
        ...
        libreTranslateUrl = config.getString("libreTranslateUrl", Configuration.CATEGORY_GENERAL, DEFAULT_ENDPOINT, "Self-hosted only: LibreTranslate base URL, used purely for language detection");
        nllbUrl = config.getString("nllbUrl", Configuration.CATEGORY_GENERAL, DEFAULT_ENDPOINT, "Self-hosted only: NLLB-200 base URL, used for the actual translation");
        selfHostedFallbackLang = config.getString("selfHostedFallbackLang", Configuration.CATEGORY_GENERAL, "", "Self-hosted only: ISO code to assume when detection is too unsure to trust. Empty (default) means skip the message instead of guessing, which is what you want on a mostly-English server");
        selfHostedMinConfidence = config.getInt("selfHostedMinConfidence", Configuration.CATEGORY_GENERAL, 20, 0, 100, "Self-hosted only: below this detection confidence the message is skipped (or the fallback language used, if one is set). Confidence tracks message length more than correctness, so keep this low: real Spanish scores about 30 on two words");
        selfHostedTimeout = config.getInt("selfHostedTimeout", Configuration.CATEGORY_GENERAL, 5000, 500, 60000, "Self-hosted only: connect and read timeout in milliseconds");
        selfHostedMinChars = config.getInt("selfHostedMinChars", Configuration.CATEGORY_GENERAL, 6, 0, 100, "Self-hosted only: skip messages with fewer than this many letters (emoji shortcodes like :sob: do not count). Short tokens are detected confidently but wrongly - olk and kk both come back as Finnish at 100 percent");
        selfHostedMaxSimilarity = config.getInt("selfHostedMaxSimilarityPercent", Configuration.CATEGORY_GENERAL, 50, 0, 100, "Self-hosted only: reject a translation that is at least this percent similar to the original, because it means the language was misdetected and nothing was really translated. Genuine translations score about 11 percent; misdetected English scores 48 and up. 100 disables the check");
        selfHostedApiKey = config.getString("selfHostedApiKey", Configuration.CATEGORY_GENERAL, DEFAULT_API_KEY, "Self-hosted only: API key sent as X-API-Key. Needed when the services sit behind the authenticating gateway (i.e. reached over the internet); leave empty for a direct LAN connection");
    ...
    public List<String> getSkipLanguages() {
        return skipLanguages;
    }

    public List<String> getServerTranslationTags() {
        return serverTranslationTags;
    }

    public boolean isServerTranslation(String messageBody) {
        return ServerTagFilter.isServerTranslated(messageBody, serverTranslationTags);
    }

    public void setSkipLanguages(List<String> skipLanguages) {
        this.skipLanguages = skipLanguages;
        String[] array = skipLanguages.toArray(new String[0]);
        config.get(Configuration.CATEGORY_GENERAL, "skipLanguages", new String[0],
                "Languages never to translate automatically, as ISO codes or English names (e.g. es, Spanish). The target language is always skipped and does not need listing. Explicit actions - signs, /tr and retranslate - ignore this list").set(array);
        config.save();
    }

    public boolean isLanguageSkipped(Language language) {
        if (language == null) {
            return false;
        }
        return LanguageFilter.isBlocked(skipLanguages, language.getGoogleCode(), language.getName());
    }

    public boolean isLanguageSkipped(String isoCode) {
        return LanguageFilter.isBlocked(skipLanguages, isoCode, null);
    }
    ...
    //what translations are actually running on right now, usually the saved one unless google died
    public String getActiveEngine() {
        return sessionEngine != null ? sessionEngine : translationEngine;
    }

    //this session only
    public void setSessionEngine(String engine) {
        this.sessionEngine = engine;
    }

    public boolean isSessionFallbackActive() {
        return sessionEngine != null;
    }

    public void setTranslationEngine(String translationEngine) {
        this.translationEngine = translationEngine;
        //picking an engine by hand ends the fallback
    ...
    public void setLibreTranslateUrl(String libreTranslateUrl) {
        this.libreTranslateUrl = libreTranslateUrl;
        config.get(Configuration.CATEGORY_GENERAL, "libreTranslateUrl", libreTranslateUrl, "Self-hosted only: LibreTranslate base URL, used purely for language detection").set(libreTranslateUrl);
        config.save();
    }

    public void setNllbUrl(String nllbUrl) {
        this.nllbUrl = nllbUrl;
        config.get(Configuration.CATEGORY_GENERAL, "nllbUrl", nllbUrl, "Self-hosted only: NLLB-200 base URL, used for the actual translation").set(nllbUrl);
        config.save();
    }

    public void setSelfHostedApiKey(String selfHostedApiKey) {
        this.selfHostedApiKey = selfHostedApiKey;
        config.get(Configuration.CATEGORY_GENERAL, "selfHostedApiKey", selfHostedApiKey, "Self-hosted only: API key sent as X-API-Key. Needed when the services sit behind the authenticating gateway (i.e. reached over the internet); leave empty for a direct LAN connection").set(selfHostedApiKey);
        config.save();
    }
    ...
    public String getLibreTranslateUrl() {
        return trimUrl(libreTranslateUrl);
    }

    public String getNllbUrl() {
        return trimUrl(nllbUrl);
    }

    public String getSelfHostedFallbackLang() {
        return selfHostedFallbackLang;
    }

    public int getSelfHostedMinConfidence() {
        return selfHostedMinConfidence;
    }

    public int getSelfHostedTimeout() {
        return selfHostedTimeout;
    }

    public int getSelfHostedMinChars() {
        return selfHostedMinChars;
    }

    //give a fraction rather than a percent
    public double getSelfHostedMaxSimilarity() {
        return selfHostedMaxSimilarity / 100.0;
    }

    public String getSelfHostedApiKey() {
        return selfHostedApiKey;
    }

    //a trailing slash would give us "//detect", so strip it
    ...
    //save, then check it actually got written.
    public void saveConfig() {
        config.save();
        if (!writtenToDisk()) {
            Log.logger.error("Could not write {}. Settings are active for this session only.",
                    configFile == null ? "the config file" : configFile.getAbsolutePath());
            ChatUtil.printChatMessage(true,
                    "Settings applied, but they could not be saved and will be lost when you quit. See the log for the file path.",
                    TextFormatting.RED);
            return;
        }
        syncConfig();
    ...
    private boolean writtenToDisk() {
        ...
        try {
            ...
            while ((line = reader.readLine()) != null) {
                if (line.contains("selfHostedApiKey")) {
                    return true;
                }
            }
            return false;
